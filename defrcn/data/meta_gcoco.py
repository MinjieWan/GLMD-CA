...
__all__ = ["register_meta_gcoco"]


def load_gcoco_json(json_file, image_root, metadata, dataset_name):
    is_shots = "shot" in dataset_name  # few-shot
    if is_shots:
        imgid2info = {}
        shot = dataset_name.split('_')[-1].split('shot')[0]
        split_dir = os.path.join('datasets/gcoco/', 'gcocosplit')
...
def register_meta_gcoco(name, metadata, imgdir, annofile):
    DatasetCatalog.register(
        name,
        lambda: load_gcoco_json(annofile, imgdir, metadata, name),
    )
