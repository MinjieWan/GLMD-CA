...
__all__ = ["register_meta_ifsod"]


def load_ifsod_json(json_file, image_root, metadata, dataset_name):
    is_shots = "shot" in dataset_name  # few-shot
    if is_shots:
        imgid2info = {}
        shot = dataset_name.split('_')[-1].split('shot')[0]
        split_dir = os.path.join('datasets/ifsod/', 'ifsodsplit')
...
def register_meta_ifsod(name, metadata, imgdir, annofile):
    DatasetCatalog.register(
        name,
        lambda: load_ifsod_json(annofile, imgdir, metadata, name),
    )
