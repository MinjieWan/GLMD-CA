...
def _get_gcoco_instances_meta():
    thing_ids = [k["id"] for k in GCOCO_CATEGORIES if k["isthing"] == 1]
    assert len(thing_ids) == 72, len(thing_ids)
    # Mapping from the incontiguous ICOCO category id to an id in [0, 82]
    thing_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(thing_ids)}
    thing_classes = [k["name"] for k in GCOCO_CATEGORIES if k["isthing"] == 1]
    ret = {
        "thing_dataset_id_to_contiguous_id": thing_dataset_id_to_contiguous_id,
        "thing_classes": thing_classes
    }
...
def _get_ifsod_instances_meta():
    thing_ids = [k["id"] for k in IFSOD_CATEGORIES]
    assert len(thing_ids) == 11, len(thing_ids)
    # Mapping from the incontiguous IFSOD category id to an id in [0, 10]
    thing_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(thing_ids)}
    thing_classes = [k["name"] for k in IFSOD_CATEGORIES]
    ret = {
        "thing_dataset_id_to_contiguous_id": thing_dataset_id_to_contiguous_id,
        "thing_classes": thing_classes,
    }
...
def _get_ifsod_fewshot_instances_meta():
    # "thing_dataset_id_to_contiguous_id", "thing_classes"
    ret = _get_ifsod_instances_meta()

    novel_ids = [k["id"] for k in IFSOD_NOVEL_CATEGORIES]
    novel_dataset_id_to_contiguous_id = {k: i for i, k in enumerate(novel_ids)}
    novel_classes = [k["name"] for k in IFSOD_NOVEL_CATEGORIES]

    base_categories = [
        k
        for k in IFSOD_CATEGORIES
...
def _get_builtin_metadata(dataset_name):
    ...
    elif dataset_name == "gcoco":
        return _get_gcoco_instances_meta()
    
    elif dataset_name == "voc_fewshot":
        return _get_voc_fewshot_instances_meta()
    
    elif dataset_name == "ifsod":
        return _get_ifsod_instances_meta()
    elif dataset_name == "ifsod_fewshot":
        return _get_ifsod_fewshot_instances_meta()
