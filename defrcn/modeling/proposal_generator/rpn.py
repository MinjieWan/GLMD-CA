...
class RPN(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.box_obj_loss_type = box_obj_loss_type
    ...
    def from_config(cls, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        ret = {
            ...
            "box_obj_loss_type": cfg.MODEL.RPN.BBOX_OBJ_LOSS_TYPE,
        }
    ...
    def losses(
        ...
        if self.box_obj_loss_type == "normal":
        ...
        # CA-RPN loss
            ...
            objectness_score = torch.stack([torch.sigmoid(pred_objectness_logits[i]) for i in range(pred_objectness_logits.shape[0])])
            score_mask = (gt_labels == 0) & (objectness_score > 0.5)
            confusion_number = (score_mask == True).sum().item()
            final_mask = valid_mask & ~score_mask
            objectness_loss = F.binary_cross_entropy_with_logits(
                pred_objectness_logits[final_mask],
                gt_labels[final_mask].to(torch.float32),
                reduction="sum",
            )
