...
class FastRCNNOutputs(object):
    ...
    def __init__(
        ...
        self.propasal_num = propasal_num
    ...
    def ca_loss(self):
        """
        confusion aware loss for box classification.  
        ...
        bg_idx = self.pred_class_logits.shape[1] - 1  # bg idx
        fg_class = self.gt_classes != bg_idx
        bg_class = self.gt_classes == bg_idx
        
        # Get the top-1 cls score idx of each proposal (excluding background)
        orin_pred_score = F.softmax(self.pred_class_logits, dim=-1)  # cls score
        _, topk_idx = torch.topk(orin_pred_score, k=2, dim=-1)  # get top-2 score (include background), [proposal_num, 2]
        # Exclude background score
        first_score_idx, second_score_idx = topk_idx[:, 0], topk_idx[:, 1]
        score_idx = torch.where(first_score_idx == bg_idx, second_score_idx, first_score_idx)

        # Create the mask for filtering the confusing negative proposals
        score_mask = torch.ones_like(orin_pred_score)
        score_mask.scatter_(dim=1, index=score_idx.unsqueeze(1), value=0.0)
        # Restore all categories for the positive proposals
        score_mask[fg_class, :] = 1
        # Restore the category in the current image for the negative proposals
        total_proposal_num = self.pred_class_logits.shape[0]
        for i in range(int(total_proposal_num / self.propasal_num)):
            start_idx = i * self.propasal_num
            end_idx = self.propasal_num + start_idx
            current_class_idx = copy.deepcopy(self.fs_class[i])

            tmp = score_mask[start_idx: end_idx, current_class_idx]
            tmp[bg_class[start_idx: end_idx],:] = 1
            score_mask[start_idx: end_idx, current_class_idx] = tmp

        pred_logits = self.pred_class_logits.clone()
        pred_logits[score_mask == 0] = -1e10
    ...
    def losses(self):
        ...
        if self.box_class_loss_type == "CA":
            return {
                "loss_cls": self.ca_loss(),
                "loss_box_reg": self.smooth_l1_loss(),
            }
        else:
            return {
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def forward(self, x):
        # Decouple the classifier and the regressor
        if isinstance(x, dict):
            x_cls = x["cls"]
            if x_cls.dim() > 2:
                x_cls = torch.flatten(x_cls, start_dim=1)
            if self._do_cls_dropout:
                x_cls = F.dropout(x_cls, self._dropout_ratio, training=self.training)
