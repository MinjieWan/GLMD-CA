...
from ..meta_arch.gdl import AffineLayer, decouple_layer
...
class Res5ROIHeads(ROIHeads):
    ...
    def __init__(self, cfg, input_shape):
        ...
        # CRD module
        if self.cfg.MODEL.ENABLE_GLMD and self.cfg.MODEL.GLMD.ENABLE_CRD:
            # self.decouple_cls_layer = nn.Sequential(AffineLayer(num_channels=input_shape['res4'].channels*2, bias=True),
            #                                         nn.ReLU())
            # self.decouple_reg_layer = nn.Sequential(AffineLayer(num_channels=input_shape['res4'].channels*2, bias=True),
            #                                         nn.ReLU())
            self.decouple_cls_layer = nn.Sequential(nn.Conv2d(out_channels, out_channels // 8, kernel_size=1),
                                                    nn.ReLU(),
                                                    nn.Conv2d(out_channels // 8, out_channels, kernel_size=1),
                                                    nn.Sigmoid())
            self.decouple_reg_layer = nn.Sequential(nn.Conv2d(out_channels, out_channels // 8, kernel_size=1),
                                                    nn.ReLU(),
                                                    nn.Conv2d(out_channels // 8, out_channels, kernel_size=1),
                                                    nn.Sigmoid())
    ...
    def forward(self, images, features, proposals, targets=None, fs_class=None):
        ...
        )  # [batch*proposal_num, channels, height, width]

        feature_pooled = box_features.mean(dim=[2, 3])

        if self.cfg.MODEL.ENABLE_GLMD and self.cfg.MODEL.GLMD.ENABLE_CRD:
            feature_pooled = feature_pooled.unsqueeze(-1).unsqueeze(-1)
            feature_cls = self.decouple_cls_layer(feature_pooled) * feature_pooled
            feature_reg = self.decouple_reg_layer(feature_pooled) * feature_pooled
            feature_pooled = {"cls": feature_cls, "reg": feature_reg}
        
        pred_class_logits, pred_proposal_deltas = self.box_predictor(
            feature_pooled
        )  # [batch*proposal_num, num_class + 1], [batch*proposal_num, num_class * 4]
        del feature_pooled
