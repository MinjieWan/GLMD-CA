...
from .glmd import GLMDNet
from defrcn.modeling.roi_heads import build_roi_heads
from defrcn.modeling.proposal_generator import build_proposal_generator
...
class GeneralizedRCNN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if cfg.MODEL.ENABLE_GLMD:
            self.glmd_network = GLMDNet(cfg, num_channels=self._SHAPE_['res4'].channels)
        self.to(self.device)
    ...
    def forward(self, batched_inputs):
        ...
        fs_class = [x["fs_class"] for x in batched_inputs]  # the ground_truth class of instance
    ...
    def _forward_once_(self, batched_inputs, gt_instances=None, fs_class=None):
        ...
        if self.cfg.MODEL.ENABLE_GLMD:
            features = {k: self.glmd_network(features[k]) for k in features}

        features_de_rpn = features
        if self.cfg.MODEL.RPN.ENABLE_DECOUPLE:
            scale = self.cfg.MODEL.RPN.BACKWARD_SCALE
            if self.cfg.MODEL.RPN.BBOX_OBJ_LOSS_TYPE == "CA":
